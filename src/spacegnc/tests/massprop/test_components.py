...
from spacegnc.massprop.components import (
    DENSITY_KEY,
    HEIGHT_KEY,
    MASS_KEY,
    MOI_KEY,
...
class TestMassPropertyNode:
    """Test MassPropertyNode object."""
    ...
    def test_parallel_axis_theorem(self, moi, rel_pos, valid_moi_at):
        ...
        test_part = MassPropertyNode.from_mass_prop_dict(test_mass_prop)
    ...
    def test_add_child_parts_cg_shift(self):
        ...
        root_part = MassPropertyNode.from_mass_prop_dict(test_root_mass_prop)
        ...
        test_child_mass_prop_1 = {
            ...
            CG_REL_POS_KEY: np.array([[1], [0], [0]]),
        }
        test_child_part_1 = MassPropertyNode.from_mass_prop_dict(test_child_mass_prop_1)

        test_child_mass_prop_2 = {
            MASS_KEY: 0.5,
            MOI_KEY: np.zeros(shape=(3, 3)),
            NAME_KEY: "TEST-FIRST-LAYER-2",
            CG_REL_POS_KEY: np.array([[0], [1], [0]]),
        }
        test_child_part_2 = MassPropertyNode.from_mass_prop_dict(test_child_mass_prop_2)

        test_child_mass_prop_3 = {
            MASS_KEY: 1.5,
            MOI_KEY: np.zeros(shape=(3, 3)),
            NAME_KEY: "TEST-SECOND-LAYER-1",
            CG_REL_POS_KEY: np.array([[0], [0], [1]]),
        }
        test_child_part_3 = MassPropertyNode.from_mass_prop_dict(test_child_mass_prop_3)
        ...
        assert (
            root_part.mass
            == test_root_mass_prop[MASS_KEY] + test_child_mass_prop_1[MASS_KEY] + test_child_mass_prop_2[MASS_KEY]
        )
        ...
        assert (
            root_part.mass
            == test_root_mass_prop[MASS_KEY]
            + test_child_mass_prop_1[MASS_KEY]
            + test_child_mass_prop_2[MASS_KEY]
            + test_child_mass_prop_3[MASS_KEY]
        )
        assert np.allclose(
            root_part.moi,
            np.array(
...
class TestCylindricalShellPart:
    ...
    def test_mass_prop_from_shape(self):
        ...
        mass_prop = CylindricalShellPart.compute_mass_prop_dict_from_shape(test_shape_prop)
        ...
        assert np.allclose(
            ...
            np.array(
                [
                    [2.9384363286576543, 0, 0],
                    [0, 2.9384363286576543, 0],
                    [0, 0, 5.03911461635803],
                ]
            ),
        )
