...
from spacegnc.constant import G0, MU_E, OMEGA_E, RADIUS_E, RHO_SEA_LEVEL_E, ScaledConstantProvider

from spacegnc.dynamics.base import DynamicsModel
...
class LaunchVehicle2D(DynamicsModel):
    """Launch vehicle inplane dynamics model.

    Simplification of a launch vehicle model in 2D space as a rigid body with restricted
    rotational dynamics with a 1D engine gimbal and throttle as control.
    """

    # Define required number of elements in the state/control vector
    # Position (2), velocity (2), pitch angle (1), pitch rate (1), mass (1)
    REQUIRED_STATE_NUM: int = 7

    # Control vector: throttle (1), gimbal angle wrt body (1)
    REQUIRED_CTRL_NUM: int = 2
...
class LaunchVehicle3DOF(DynamicsModel):
    """Launch vehicle translational dynamics model with 3 degree of freedoms.
...
class LaunchVehicle6DOF(DynamicsModel):
    """Launch vehicle 3D dynamics model with 6 degree of freedoms.
