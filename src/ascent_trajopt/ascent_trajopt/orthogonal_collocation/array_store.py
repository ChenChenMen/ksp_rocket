...
class OptimizationArray(np.ndarray):
    ...
    def __new__(
        ...
        if casted_array.shape[0] != expected_length:
            raise ValueError(
                ...
                f"from discretizer with {casted_array.discretizer.total_num_points} points for optimization "
                f"and point dimension of {point_dimension}."
            )
    ...
    def point_index_slice(discretizer: HPDiscretizer, dimension: DynamicModelDimension, point_index: int) -> tuple[int, int]:
        ...
        end_index = (point_index + 1) * point_dimension - 1
    ...
    def segment_index_slice(discretizer: HPDiscretizer, segment_index: int) -> tuple[int, int]:
        ...
        end_point_index = num_point_rolling_collection[segment_index] - 1
    ...
    def segment_point_index_slice(
        ...
        _, end_index = OptimizationArray.point_index_slice(discretizer, dimension, end_point_index)
        return start_index, end_index
    ...
    def point(self, point_index: int) -> DynamicVariablesArray:
        ...
        return DynamicVariablesArray(self[start_index : end_index + 1], dimension=self.dimension)
    ...
    def segment(self, segment_index: int) -> list[DynamicVariablesArray]:
        ...
        return [self.point(idx) for idx in range(start_point_index, end_point_index + 1)]
