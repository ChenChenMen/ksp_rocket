...
_COEFFICIENT_CACHE_INDEX_FILENAME = "cached_index.json"


def _get_orthopoly_cache_content(filename: str):
    """Retrieve the cached value by key."""
    cache_file_path = _CACHE_PATH / filename
    return json.loads(cache_file_path.read_text()) if cache_file_path.exists() and cache_file_path.is_file() else {}
...
def with_orthopoly_cache(key: str = None):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if not key and (cache_index := _get_orthopoly_cache_content(filename=_COEFFICIENT_CACHE_INDEX_FILENAME).get(cache_key)):
            ...
            json.dump(cache_index_dict, cache_file_path.open("w"))

            # Save the latest value to the cache
            cache_file_path = _CACHE_PATH / cache_filename
            json.dump(result, cache_file_path.open("w"))
...
def get_legendre_gauss_radau_points(nth: int):
    ...
    real_roots = np.sort(roots.real[roots.imag == 0]).tolist()
